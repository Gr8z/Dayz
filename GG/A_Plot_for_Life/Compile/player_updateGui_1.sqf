...
_ctrlCombatBorder ctrlSetTextColor [0.8,0.8,0.8,1];
_ctrlTempBorder ctrlSetTextColor [1,1,1,1];
//_ctrlWeight ctrlSetTextColor [1,1,1,1];
...
_humanityHUD = (player getVariable ['humanity',0]);
			_colorHUD = [1,1,1,1];	
			if(_humanityHUD < -5000) then {
				_colorHUD = [0.8,0.07,0.06,1];
			} else {
				if(_humanityHUD > 5000) then {
					_colorHUD = [0,0.6,1,1];
				};
			};
_ctrlHumanityAmount ctrlSetTextColor (_colorHUD);	
_ctrlHumanityAmount ctrlSetText str(_humanityHUD);
...
if (_tempVal > 0.8) then {_ctrlTemp call player_guiControlFlash;
} else {_ctrlTemp ctrlShow true;};
