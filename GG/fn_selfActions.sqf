...
if ((animationState player) in ["ActsPercMstpSnonWnonDnon_DancingDuoIvan","ActsPercMstpSnonWnonDnon_DancingDuoStefan","ActsPercMstpSnonWnonDnon_DancingStefan","AmovPercMstpSnonWnonDnon_idle68boxing","AmovPercMstpSnonWnonDnon_exerciseKata","AmovPercMstpSnonWnonDnon_exercisePushup","AmovPercMstpSnonWnonDnon_exercisekneeBendA","AmovPercMstpSnonWnonDnon_idle70chozeniPoRukou","smk_urbanproneright","smk_prone_to_urbanprone_right","smk_urbanproneleft","smk_prone_to_urbanprone_left"]) then {
	...
	systemChat "You can't do these awesome animations near Plot Poles!";
	};
};
...
if (!isNull _nearLight) then {
    ...
    if (_inVehicle && driver _vehicle == player) then {
		if (s_player_inCarlockUnlock_crtl < 0) then {
			...
			_lock = dayz_myLockedVehicle addAction [format[localize "STR_EPOCH_ACTIONS_LOCK",_text], "GG\vehicle_lockUnlock.sqf", [dayz_myLockedVehicle, true], 1, false, true, "", ""];
...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
		...
		if (s_player_plot_take_ownership < 0) then {
			if (DZE_PlotOwnership) then {
				...
				If (( _isowner select 0 )) then{
					s_player_plot_take_ownership = player addAction ["Take plot items ownership", "GG\A_Plot_for_Life\Action\plot_take_ownership.sqf", "", 1, false];
				};
			};
		};
	 } else {
	...
	if (!_isAlive and !_isZombie and !_isAnimal and _isMan and _canDo) then {
        if (s_player_bury_human < 0) then {
            s_player_bury_human = player addAction [format["Bury Body"], "GG\bury_human.sqf",cursorTarget, 0, false, true, "", ""];
        }
    } else {
    ...
    if (_isMan and !_isAlive and !_isZombie and !_clothesTaken) then {
        if (s_player_clothes < 0) then {
            s_player_clothes = player addAction [("<t color='#0096ff'>")+("Take Clothes")+("</t>"), "GG\player_takeClothes.sqf",[_cursorTarget], -10, false, true, "",""];
        };
    } else {
	...
	if(_isTent && (player distance _cursorTarget < 3)) then {
		if (_ownerID == _playerUID) then {
			...
			if(("ItemJerrycan" in _magazinesPlayer) && ("ItemMatchbox_DZE" in weapons player)) then {
				if (s_player_packtent < 0) then {
					s_player_packtent = player addAction [localize "STR_EPOCH_ACTIONS_DESTROYTENT", "GG\A_Plot_for_Life\Action\remove.sqf",_cursorTarget, 1, true, true, "", ""];
				};
			};
		};
	} else {
	...
	if((_typeOfCursorTarget in DZE_DoorsLocked)) then {
		if (s_player_manageDoor < 0) then {		 
		 s_player_manageDoor = player addAction ["<t color='#0059FF'>Manage Door</t>", "GG\doorManagement\initDoorManagement.sqf", _cursorTarget, 5, false];
		};
	} else {
	...
	*/
	...
	if(_typeOfCursorTarget in DZE_ATM  and (player distance _cursorTarget < 3)) then {		
		if (s_bank_dialog2 < 0) then {
			s_bank_dialog2 = player addAction ["ATM", "GG\gold\bank_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
		};
	} else {		
	...
	if (_isMan and _isAlive and !_isZombie and !_isAnimal  and !_isError and !(_traderType in serverTraders)) then {
		if (s_givemoney_dialog < 0) then {
			s_givemoney_dialog = player addAction [format["Give Coins to %1", (name _cursorTarget)], "GG\gold\give_player_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
		};
	} else {
	...
	if ((_cursorTarget isKindOf "ModularItems") || (_cursorTarget isKindOf "Land_DZE_WoodDoor_Base") || (_cursorTarget isKindOf "CinderWallDoor_DZ_Base")) then {
		...
		if (s_player_upgrade_build < 0) then {
			...
			s_player_upgrade_build = player addAction [format[localize "STR_EPOCH_ACTIONS_UPGRADE",_text], "GG\A_Plot_for_Life\Action\player_upgrade.sqf",_cursorTarget, -1, false, true, "",""];
		};
	} else {
		player removeAction s_player_upgrade_build;
		s_player_upgrade_build = -1;
	...
	if((_isDestructable || _cursorTarget isKindOf "Land_DZE_WoodDoorLocked_Base" || _cursorTarget isKindOf "CinderWallDoorLocked_DZ_Base") && (DZE_Lock_Door == _characterID)) then {
		...
		if (s_player_downgrade_build < 0) then {
			...
			s_player_downgrade_build = player addAction [format[localize "STR_EPOCH_ACTIONS_REMLOCK",_text], "GG\A_Plot_for_Life\Action\player_buildingDowngrade.sqf",_cursorTarget, -2, false, true, "",""];
		};
	} else {
		player removeAction s_player_downgrade_build;
		s_player_downgrade_build = -1;
...
if(Player_Vehicle_Crew_Count != (count _crew)) then {
	 ...
	 
	{
		...
		s_player_driver_eject = _vehicle addaction[_eject_text,"GG\eject.sqf",[_x,_ride],0,false,true,"", "(driver _target)==player"];
