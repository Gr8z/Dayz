...
};
{
	...
	if (_damage > 0) then {
		...
		_string = fmt["<t %2>Repair%1 (%3 %4)</t>",_cmpt,_color,_percent,"%"];
		_handle = dayz_myCursorTarget xaa [_string, "\z\addons\dayz_code\actions\repair.sqf",[_vehicle,_part,_x], 0, false, true, "",""];
		s_player_repairActions set [count s_player_repairActions,_handle];
	};
} forEach _hitpoints;
if (_allFixed) then {_vehicle setDamage 0};
