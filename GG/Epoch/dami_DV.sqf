...
xcc ("
	...
	if ("+_veh+"_DEPLOYED > "+str _max+") exitWith {systemChat ('(ArmA-AH): '+str (format ['You have built %1 out of a maximum of "+str _max+" "+_typ+".',"+_veh+"_DEPLOYED]))};
	if (dayz_combat == 1) exitWith {systemChat ('(ArmA-AH): '+str ('You can''t deploy vehicles while in combat!'))};
	_required = "+str _req+";
	...
	if (count _required <= 0) then {
		...
		systemChat ('(ArmA-AH): '+str (format ['You have built a "+_typ+"! You have built %1 out of a maximum of %2 "+_typ+".!',"+_veh+"_DEPLOYED,"+str _max+"]));
		'<t size =''1'' font=''Zeppelin33'' color=''#FF0000''>Deployed vehicles disappear after server restart!</t>' call AH_fnc_dynTextMsg;
		player switchMove '';
	} else {systemChat ('(ArmA-AH): '+str ('You need: '+str _required+' to build "+_typ+".'))};
");
