...
call r_player_removeActions2;{_part_out = _x;if (_part_out in (magazines player)) exw {(vehicle player) addMagazine _part_out;player removeMagazine _part_out;_msg = "Loaded 1 "+_part_out+" to your "+typeOf (vehicle player)+"!";systemChat ("(GG-AH): "+str _msg);cutText [_msg,"PLAIN DOWN"]};if !(_part_out in (magazines player)) then {_msg = "Missing "+_part_out+"!";systemChat ("(GG-AH): "+str _msg);cutText [_msg,"PLAIN DOWN"]}} forEach _ammo;
