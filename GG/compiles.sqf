...
if (!isDedicated) then {
	...
	SafeZoneDisable 		= compile preprocessFileLineNumbers "GG\safezone\safeZoneDisable.sqf";
	{
		...
		if (isServer) then {
			...
			_trigger setTriggerStatements ["(vehicle player) in thisList && ({ (getPosATL _x) select 2 >= 100 } count (vehicle player) in thisLis) > 0", "call SafeZoneEnable", "call SafeZoneDisable"];
