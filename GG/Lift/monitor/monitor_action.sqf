private ["_target","_transportable","_addedVehs","_allVehicles"];
...
while {true} do{
	...
	if ( !(isNull _target) && ( player distance _target < 13 ) ) then{
		...
		if ({_target isKindOf _x} count LOG_CFG_ISTOWABLE > 0) then {
			LOG_OBJECT_TRAILER_VALID = (
				(vehicle player == player) && (alive _target) && (count crew _target == 0) &&
			...
			LOG_DETACH_VALID = (
				(vehicle player == player) && (isNull LOG_OBJECT_MOVES) &&
				(!isNull (_target getVariable "LOG_moves_by")) && !(_target getVariable "LOG_disabled")
			);
		};
		if ({_target isKindOf _x} count LOG_CFG_CANTOW > 0) then {
			LOG_TRAILER_MOVE_VALID = (
				(vehicle player == player) && (alive _target) && (!isNull LOG_OBJECT_MOVES) &&
	...
	if (vehicle player != player) then {
		if ({(vehicle player) isKindOf _x} count LOG_CFG_CANLIFT > 0) then{
			...
			LOG_HELI_LIFT_VALID = (
				(driver LOG_OBJECT_ADDACTION == player) &&
			...
			LOG_HELI_DROP_VALID = (
				(driver LOG_OBJECT_ADDACTION == player) &&
				(!isNull (LOG_OBJECT_ADDACTION getVariable "LOG_heliporte")) && ([0,0,0] distance velocity LOG_OBJECT_ADDACTION <= 10 ) && (getPos LOG_OBJECT_ADDACTION select 2 <= 40) &&
				!(LOG_OBJECT_ADDACTION getVariable "LOG_disabled")
			);
		};
	};
	if ((time - _lastloop) > 10) then {
		{if ((count (units _x) == 0)&&(!isNull _x)) then {deleteGroup _x}} forEach allGroups;
	};
	if ((time - _lastloop) > 30) then {
		_lastloop = time;
		_allVehicles = ((allMissionObjects "Air")+(allMissionObjects "Ship")+(allMissionObjects "LandVehicle")) - _addedVehs;
		if ((count _allVehicles) > 0) then {
			{
				_object = _x;
				if ({_object isKindOf _x} count _transportable > 0) then{[_object] call LOG_OBJ_INIT};			
				if ({_object isKindOf _x} count LOG_CFG_CANLIFT > 0) then{[_object] call LOG_LIFT_INIT};	
				if ({_object isKindOf _x} count LOG_CFG_CANTOW > 0) then{[_object] call LOG_TOW_INIT};
			} forEach _allVehicles;
			_addedVehs = _addedVehs + _allVehicles;
		};
	};
	sleep 1;
};