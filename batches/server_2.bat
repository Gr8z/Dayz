...
cd /d %serverkill%
taskkill /im server_2.exe
echo.
echo Kill Bec1.exe
set beckill="E:\A2Server\BEC"
cd /d %beckill%
taskkill /im Bec_2.exe
timeout 2
echo.
...
timeout 2
:: start the server..
start /REALTIME "arma2" /min "E:\A2Server\server_2.exe" -port=2322 "-config=server_2\config.cfg" "-cfg=server_2\basic.cfg" "-profiles=server_2" -name=server_2 "-mod=@DayzOverwatch;@DayZ_Epoch;@Server_2;" "-BEpath=E:\A2Server\BattlEye" "-malloc=tbb3malloc_bi"  -world=tavi -cpuCount=4 -exThreads=1 -maxmem=2047 -noCB
echo.
echo Starting Bec
timeout 2
:: start bec
