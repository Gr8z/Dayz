...
timeout 1
start /wait /REALTIME "arma2" /min "E:\A2Server\server_test.exe" "-port=2362" "-config=server_test\config.cfg" "-mod=@Taviana;@DayzOverwatch;@DayZ_Epoch;@server_test;" "-cfg=server_test\basic.cfg" "-profiles=server_test" "-name=server_test" "-malloc=tbb3malloc_bi"  -world=tavi -cpuCount=4 -exThreads=1 -maxmem=2047 -noCB
echo.
echo.
cls
@exit