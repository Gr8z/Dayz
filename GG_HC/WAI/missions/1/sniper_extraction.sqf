...
_mission = [_position,"Medium",format["Civilian %1",_vehname],"MainHero",true] call mission_init;

diag_log format["WAI: [Mission:[Hero] civil Vehicle]: Starting... %1",_position];
...
[[_position select 0,_position select 1,0],2,"Medium","Random",3,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
[[_position select 0,_position select 1,0],2,"Medium","Random",3,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
...
if (debug_mode) then {
	diag_log format["WAI: [Hero] civil_vehicle spawned a %1",_vehname];
};
...
_complete = [
	...
	"Bandits have disabled a civilian vehicle with some vehicle ammo nearby! Check your map for the location!",	// mission announcement
	"The civilian vehicle has been secured, great job!",																		// mission success
	"The civilian vehicle has not been secured in time!"																// mission fail
] call mission_winorfail;

if (_complete) then {[_crate,2,2,[12,crate_items_chainbullets],2] call dynamic_crate};
diag_log format["WAI: [Mission:[Hero] civil vehicle ended at %1",_position];
