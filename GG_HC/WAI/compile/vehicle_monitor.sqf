...
_veh = _this select 0;
if (count _this == 2) then {_mission = _this select 1} else {_mission = nil};
...
waitUntil {
	...
	{if ((isPlayer _x) && (_x distance _veh <= 750)) exitWith {_player_present = true}} count _allPlayers;
	(!_player_present)
};
{deleteVehicle _x} count (crew _veh) + [_veh];