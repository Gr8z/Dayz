...
waitUntil
{
	...
	{if ((isPlayer _x) && (_x distance [_pos_x,_pos_y,0] <= _triggerdis)) then {_player_present = true}} count _allPlayers;
	(_player_present)
};
...
if (!isNil "_mission") then {
	...
	_missionrunning = (typeName (wai_mission_data2 select _mission) == "ARRAY");
	...
	if (_helipos distance [(_position select 0),(_position select 1),100] <= 200) then {
		...
		for "_x" from 1 to _paranumber do {
			...
			if (!isNil "_mission") then {
				...
				_mission_data = (wai_mission_data2 select _mission);

				if (typeName _mission_data == "ARRAY") then {
					_ainum = _mission_data select 0;
					wai_mission_data2 select _mission set [0, (_ainum + 1)];
