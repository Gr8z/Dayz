...
while {!_timeout && !_complete} do {
	...
	if (_currenttime - _starttime >= _timeout_time) then {_timeout = true};
	...
	call {
		if (_type == "crate") exitWith {
			if (wai_kill_percent == 0) then {
				...
				if (((wai_mission_data2 select _mission) select 0) <= _killpercent) then {
					_allPlayers = playableUnits;
					{
						if ((isPlayer _x) && (_x distance _position <= 20)) then {
							_complete = true
		...
		if (_type == "kill") exitWith {
			if (((wai_mission_data2 select _mission) select 0) == 0) then {_complete = true};
		};
	...
	if (wai_clean_mission) then {
		[_position,_baseclean] spawn {
			...
			while {!_cleaned} do {
				_playernear = false;
				_allPlayers = playableUnits;
				{if ((isPlayer _x) && (_x distance _pos < 400)) exitWith {_playernear = true}} count _allPlayers;	
				_currenttime = time;
				if ((_currenttime - _finish_time >= wai_clean_mission_time) && !_playernear) then {
					{if (typeName _x == "ARRAY") then [{{if ((_x getVariable ["ObjectID", nil]) == nil) then {deleteVehicle _x}} count _x},{if ((_x getVariable ["ObjectID", nil]) == nil) then {deleteVehicle _x}}]} forEach _clean;
					_cleaned = true;
				};
				sleep 1;
			};
...
if (_timeout) then {
		...
		if (_x getVariable ["mission", nil] == _mission) then {
			if (alive _x) then {
				...
				if (!isNil "_cleanunits") then {
					call {
						if (_cleanunits == "ground2") 	exitWith {ai_ground_units = (ai_ground_units -1)};
						if (_cleanunits == "air2") 		exitWith {ai_air_units = (ai_air_units -1)};
						if (_cleanunits == "vehicle2") 	exitWith {ai_vehicle_units = (ai_vehicle_units -1)};
						if (_cleanunits == "static2") 	exitWith {ai_emplacement_units = (ai_emplacement_units -1)};
					};
				};
			};
			deleteVehicle _x;
	...
	} forEach _baseclean + ((wai_mission_data2 select _mission) select 2) + [_crate];
