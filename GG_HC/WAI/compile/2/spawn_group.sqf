...
if (count _this > 9) then {_mission = _this select 9} else {_mission = nil};
...
for "_x" from 1 to _unitnumber do {
	...
	if (_backpack != "none") then {_unit addBackpack _aipack};
	...
	if (!isNil "_mission") then {
		_ainum = (wai_mission_data2 select _mission) select 0;
		wai_mission_data2 select _mission set [0, (_ainum + 1)];
		_unit setVariable ["missionclean", "ground2"];
