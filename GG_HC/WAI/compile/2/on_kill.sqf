...
if (!isNil "_mission") then {
	if (typeName(wai_mission_data2 select _mission) == "ARRAY") then {
		wai_mission_data2 select _mission set [0, ((wai_mission_data2 select _mission) select 0) - 1];
	};
};
...
if (ai_add_skin) then {
	_skin = (typeOf _unit);
	_skin = "Skin_" + _skin;
	if (isClass (configFile >> "CfgMagazines" >> _skin)) then {[_unit,_skin] call BIS_fnc_invAdd};
};
if (isPlayer _player) then {
	...
	if (ai_share_info) then {
		{if (((position _x) distance (position _unit)) <= ai_share_distance) then {_x reveal [_player, 4.0]}} count allUnits;
	};
} else {
	if (ai_clean_roadkill) then {
		...
		{_unit removeMagazine _x} count magazines _unit;
	} else {if ((random 100) <= ai_roadkill_damageweapon) then {removeAllWeapons _unit}};
};
if (wai_remove_launcher && _launcher != "") then {
	_rockets = _launcher call find_suitable_ammunition;
	_unit removeWeapon _launcher;
	{if (_x == _rockets) then {_unit removeMagazine _x}} count magazines _unit;
};
if (_unit hasWeapon "NVGoggles" && ceil(random 100) < 20) then {
