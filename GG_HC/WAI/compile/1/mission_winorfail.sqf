...
while {!_timeout && !_complete} do {
	...
	call {
		...
		if (_type == "kill") exitWith {
			if (((wai_mission_data select _mission) select 0) == 0) then {_complete = true};
		};
		...
		[_position,_baseclean] spawn {
			...
			while {!_cleaned} do {
				...
				if ((_currenttime - _finish_time >= wai_clean_mission_time) && !_playernear) then {
					{
						if (typeName _x == "ARRAY") then {
							{if ((_x getVariable ["ObjectID", nil]) == nil) then {deleteVehicle _x}} count _x
						} else {
							if ((_x getVariable ["ObjectID", nil]) == nil) then {deleteVehicle _x}
						};
					} forEach _clean;
					_cleaned = true;
				};
...
if (_timeout) then {
	...
	{if (typeName _x == "ARRAY") then [{{deleteVehicle _x} count _x},{deleteVehicle _x}]} forEach _baseclean + ((wai_mission_data select _mission) select 2) + [_crate];
