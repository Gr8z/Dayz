...
AND `CharacterID` = '0'
AND `Classname`
NOT REGEXP 'barrier|storage|shed|bench|wall|floor|fence|pump|wood|hrescue|stick|pole|generator|panel|house|rack|bag|stand|barrel|canvas|wire|hedgehog|net|trap|ramp|fort|sand|scaffold|nest|killhouse|villa|castle|pub|shelter|stodola|garage';
...
UPDATE `object_data_3`
SET `object_data_3`.`CharacterID` = 0
WHERE `object_data_3`.`CharacterID` <> 0
AND`object_data_3`.`CharacterID` <= 12500
AND`object_data_3`.`Classname` NOT LIKE 'Tent%'
AND`object_data_3`.`Classname` NOT LIKE '%Locked'
AND`object_data_3`.`Classname` NOT LIKE 'Land%'
AND`object_data_3`.`Classname` NOT LIKE 'Cinder%'
AND`object_data_3`.`Classname` NOT LIKE 'Wood%'
AND`object_data_3`.`Classname` NOT LIKE 'Metal%'
AND`object_data_3`.`Classname` NOT LIKE '%Storage%'
AND`object_data_3`.`Classname` NOT IN ('OutHouse_DZ', 'GunRack_DZ', 'WorkBench_DZ', 'Sandbag1_DZ', 'FireBarrel_DZ', 'DesertCamoNet_DZ', 'StickFence_DZ', 'LightPole_DZ', 'DeerStand_DZ', 'ForestLargeCamoNet_DZ', 'Plastic_Pole_EP1_DZ', 'Hedgehog_DZ', 'FuelPump_DZ', 'Fort_RazorWire', 'SandNest_DZ', 'ForestCamoNet_DZ', 'Fence_corrugated_DZ', 'CanvasHut_DZ', 'Generator_DZ')
AND FindVehicleKeysCount3(object_data_3.CharacterID) = 0;