...
if (isServer and isNil "sm_done") then {
	...
	for "_i" from 1 to 5 do {
		...
		if ((((isnil "_hiveResponse") or {(typeName _hiveResponse != "ARRAY")}) or {((typeName (_hiveResponse select 1)) != "SCALAR")})) then {
	...
	if ((_hiveResponse select 0) == "ObjectStreamStart") then {
		...
		for "_i" from 1 to _objectCount do {
			...
			private "_class";
			_class = (_hiveResponse select 2);
			if ((_class isKindOf "AIR")||(_class isKindOf "LandVehicle")||(_class isKindOf "Ship")) then {
		...
		diag_log ("HIVE: got " + str(_bQty) + " Epoch Objects and " + str(_qtyv) + " Vehicles");
	};
						...
						if (_object distance _x < 90) then {
							_object spawn {
								...
								if (_this distance _oldPos < 5) then {
									[nil, nil, rSpawn, ["Vehicle "+typeOf _this+" has been deleted due to being left in a safezone after a restart."], {systemChat ("Server:: "+(_this select 0)+".")}] call RE;
	...
	if (isDedicated) then {
		...
		if (isnil "spawnMarkerCount") then {spawnMarkerCount = 10};
		actualSpawnMarkerCount = 0;
		for "_i" from 0 to spawnMarkerCount do {
			if !([(getMarkerPos format["newspawn_%1",_i]), [0,0,0]] call BIS_fnc_areEqual) then {
