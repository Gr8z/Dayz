server_checkLoop = {
	...
	while {_i <= 4} do {
		...
		if !(_check) exitWith {
			...
			{
				if ((getPlayerUID _x) in (levelone+leveltwo+levelthree)) then {
...
prevent_dupe = {
	...
	switch (_type) do {
		...
		case "dcdupe": {
			...
			{
				if ((getPlayerUID _x) in (levelone+leveltwo+levelthree)) then { // <--- Put here some admin UIDs to receive a message ingame.
					PlayerCheckDupe = [toArray _txt,"sChat",_x];
					(owner _player) publicVariableClient "PlayerCheckDupe";
				};
			} forEach playableUnits;
