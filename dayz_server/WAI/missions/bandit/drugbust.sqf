if(isServer) then {
	private 		["_complete","_crate_type","_mission","_playerPresent","_position","_crate","_baserunover","_dir"];
...
_baserunover23 = createVehicle ["MAP_t_salix2s", [(_position select 0) - 24, (_position select 1) - 0.01,-0.01],[], 0, "CAN_COLLIDE"];
	...
	[[(_position select 0) + 18, (_position select 1) + 25,0],3,"Medium","Random",4,"Random","Hero","Random","Hero",_mission] call spawn_group;
	[[_position select 0,_position select 1,0],3,"Medium",["Random","AT"],4,"Random","Hero","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) - 22, (_position select 1) - 56, 0],[(_position select 0) + 22, (_position select 1) + 56, 0],50,2,"ArmoredSUV_PMC","easy","Hero","Hero",_mission] call vehicle_patrol;
	[[(_position select 0),(_position select 1),0],[0,0,0],800,"AH6J_EP1",1,"Random",["Random","AT"],4,"Random","Hero","Random","Hero",false,_mission] spawn heli_para;
	...
	_complete = [
		...
		"Police are about to raid a drug dealer! Check your map for the location!",
		"Bandits have secured the drugs!",
		"Bandits did not secure the drugs in time"
