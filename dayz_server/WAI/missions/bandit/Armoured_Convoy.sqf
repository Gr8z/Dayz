if(isServer) then {
	...
	[_mission,_position,"Extreme","Armoured Convoy","MainBandit",true] call mission_init;
	diag_log 		format["WAI: [Mission:[Bandit] Armoured Convoy]: Starting... %1",_position];
	...
	//Troops
	[[_position select 0,_position select 1,0],7,"Extreme",["Random","AT"],4,"Random","Hero","Random","Hero",_mission] call spawn_group;
	[[_position select 0,_position select 1,0],7,"Extreme",["Random","AA"],4,"Random","Hero","Random","Hero",_mission] call spawn_group;

	//Humvee Patrol
	[[(_position select 0) + 100, _position select 1, 0],[(_position select 0) + 100, _position select 1, 0],100,2,"HMMWV_M998_crows_M2_DES_EP1","Extreme","Hero","Hero",_mission] call vehicle_patrol;
	[[(_position select 0) + 120,  _position select 1, 0],[(_position select 0) + 120, _position select 1, 0],100,2,"BRDM2_TK_GUE_EP1","Extreme","Hero","Hero",_mission] call vehicle_patrol;
	[[(_position select 0) +80 ,  _position select 1, 0],[(_position select 0) +80, _position select 1, 0],100,2,"HMMWV_M1151_M2_CZ_DES_EP1_DZE","Extreme","Hero","Hero",_mission] call vehicle_patrol;
	...
	//Heli patrol
	[[(_position select 0) + 100, _position select 1, 0],[(_position select 0) + 100, _position select 1, 0],100,2,"UH1Y","Extreme","Hero","Hero",_mission] call heli_patrol;
