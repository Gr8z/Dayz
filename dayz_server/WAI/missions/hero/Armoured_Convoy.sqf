if(isServer) then {
	...
	//Troops
	[[_position select 0,_position select 1,0],7,"Extreme",["Random","AT"],4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	...
	//Heli patrol
	[[(_position select 0) + 100, _position select 1, 0],[(_position select 0) + 100, _position select 1, 0],100,2,"UH1Y_DZ","Extreme","Bandit","Bandit",_mission] call heli_patrol;
	...
	if(_complete) then {
		[_crate,[10,crate_weapons_convoy],[8,crate_tools_sniper],[15,crate_items_convoy],[4,crate_backpacks_large]] call dynamic_crate;
	};
