...
if ((count _this) > 4) then {_playerwasNearby =	_this select 4};
if (isNull _character) exitWith {diag_log ("Player is Null FAILED: Exiting, player sync: " + str(_character))};
...
if (_distance < 2000) exitWith { diag_log format["ERROR: server_playerSync: Cannot Sync Player %1 [%2]. Position in debug! %3",_name,_characterID,_charPos]};
...
if (_characterID != "0") then {
	...
	if (_isNewPos or _force) then {
		if (((_charPos select 0) == 0)&&((_charPos select 1) == 0)) then {} else {
	...
	if (_characterID != "0") then {
		...
		if (_currentModel == _modelChk) then {_currentModel = ""} else {_character setVariable ["model_CHK",_currentModel]};
		...
		if (_isInVehicle) then {_currentWpn = ""} else {
		...
		if (_timeSince > 0) then {_character setVariable ["lastTime",(time - _timeLeft)]};
	};
};
