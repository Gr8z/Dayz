...
while {1 == 1} do {
	...
	_lootTable = (if (_crashModel == "Mass_grave_DZ") then {"MassGrave"} else {["Military","HeliCrash","MilitarySpecial"] call BIS_fnc_selectRandom});
	...
	while {time < _timeToSpawn} do {uiSleep 5};
	_spawnRoll = random 1;
	if (_spawnRoll <= _spawnChance) then {
		...
		_config 	= (if (DZE_MissionLootTable) then {missionConfigFile >> "CfgBuildingLoot" >> _lootTable} else {configFile >> "CfgBuildingLoot" >> _lootTable});
