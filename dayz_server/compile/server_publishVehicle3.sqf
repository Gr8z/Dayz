...
[_object,_uid,_characterID,_class,_dir,_location,_donotusekey,_activatingPlayer] spawn {
	...
	_countr = 0;
	{_object addWeaponCargoGlobal [_x,((_weapons select 1) select _countr)];_countr = _countr + 1} count (_weapons select 0);
	_countr = 0;
	{_object addWeaponCargoGlobal [_x,((_magazines select 1) select _countr)];_countr = _countr + 1} count (_magazines select 0);
	_countr = 0;
	{_object addWeaponCargoGlobal [_x,((_backpacks select 1) select _countr)];_countr = _countr + 1} count (_backpacks select 0);
