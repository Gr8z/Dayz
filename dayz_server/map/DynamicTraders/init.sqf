...
DT_fnc_ObjectsMapper = compile preprocessFileLineNumbers "\z\addons\dayz_server\map\DynamicTraders\objectMapper.sqf";
DT_fnc_CreateTrader = compile preprocessFileLineNumbers "\z\addons\dayz_server\map\DynamicTraders\createTrader.sqf";
...
waitUntil { sleep 1; !isNil "sm_done" };
{
	for [{_i=0}, {_i<(_x select 0)}, {_i=_i+1}] do {
		...
		if ((count _position) == 2) then { 
			...
			_position execVM format["\z\addons\dayz_server\map\DynamicTraders\traders\%1", _x select 1];
